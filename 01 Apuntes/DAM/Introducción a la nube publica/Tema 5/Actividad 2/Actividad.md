...
print('\n--- HELP (lista de providers) ---')

print(json.dumps(lambda_handler({key1Name: 'help'}, None), indent=2, ensure_ascii=False))

  

print('\n--- GENERAR 3 nombres (provider: name) ---')

print(json.dumps(lambda_handler({key1Name: 'name', key2Name: 3}, None), indent=2, ensure_ascii=False))

  

print('\n--- GENERAR 5 correos electrónicos (provider: email) ---')

print(json.dumps(lambda_handler({key1Name: 'email', key2Name: 5}, None), indent=2, ensure_ascii=False))
```
